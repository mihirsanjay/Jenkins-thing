...
DWORD EntryPoint() {
  ...
  return 0;
                                          ...
                                          // What’s a relocation block?
                                          // A block describes:

                                          // One 4KB memory page in the PE image (e.g., .text, .data).

                                          // All the addresses inside that page that must be relocated (i.e., have absolute addresses).

                                          // This structure:

                                          // typedef struct _IMAGE_BASE_RELOCATION {
                                          //     DWORD VirtualAddress;   // RVA of the page
                                          //     DWORD SizeOfBlock;      // Total size of this block in bytes
                                          //     // Followed by an array of USHORTs
...
void PerformBaseRelocation(PVOID localImage, PIMAGE_NT_HEADERS ntHeader,
  ...
  PIMAGE_BASE_RELOCATION relocationTable =// absolute memory base (starting address) address of the PE fule loadaed into memory!
  ...
  while (relocationTable->SizeOfBlock > 0) {
    ...
    DWORD relocationEntriesCount =
        ...
        sizeof(USHORT);
    PBASE_RELOCATION_ENTRY relocationRVA = // ppoints to a list of ushort entries that need to be relocated
    ...
    for (DWORD i = 0; i < relocationEntriesCount; i++) {
      if (relocationRVA[i].Offset) {
        ...
        PDWORD_PTR patchedAddress = // new address of the relocation entry; everything is shifted using the address of where the pe file is loaded in memory ,i.e localImage
    ...
    // The reolcation table is filled w relocation blocks. The next relocation block is now processed.
...
bool InjectPEIntoProcess(DWORD pid) {
  ...
  printf("[*] localImage:         0x%p\n", localImage);
  printf("[*] targetImage:        0x%p\n", targetImage);
  printf("[*] deltaImageBase:     0x%p\n", deltaImageBase);
  printf("[*] EntryPoint address: 0x%p\n", (DWORD_PTR)EntryPoint);
  printf("[*] Thread start addr:  0x%p\n", (DWORD_PTR)EntryPoint + deltaImageBase);
